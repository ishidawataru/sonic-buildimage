        ...
        if state != prev_state:
            for i in lanes:
                ...
                if state != self.State.Deactivated:
                    self._eeprom.write(f"{consts.DPINIT_PENDING}{i+1}", 0)  # flag down DP Pending
        ...
        while True:
            ...
            match self._state:
                ...
                case self.State.Ready:
                    ...
                    dp_state_fields = [consts.DATAPATH_DEINIT_FIELD, consts.TX_DISABLE_FIELD]
...
#        print(
#            f"read: bank: {req.bank}, page: {req.page:02X}h, offset: {req.offset}, length: {req.length}, data: {data if len(data) != 1 else bin(data[0])}"
#        )
...
#        )
    ...
    async def GetInfo(self, req, context):
        if req.index not in self.xcvrs:
            self.xcvrs[req.index] = CMISTransceiver(req.index)
        xcvr = self.xcvrs[req.index]
        dpsms = [pb2.DataPathStateMachine(dpid=v._dpid, appsel=v._appsel, state=str(v._state)) for v in xcvr._dpsms.values()]
